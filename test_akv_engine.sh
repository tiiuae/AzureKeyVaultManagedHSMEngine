...
TEST_BINARY="test_binary.efi"
CERT_FILE="uefi-signing-cert.pem"
SIG_FILE="test_data.sig"
SIGNED_BINARY="test_binary_signed.efi"
...
cleanup_test_files() {
    ...
    rm -f "$TEST_FILE" "$TEST_BINARY" "$CERT_FILE" "$SIG_FILE" "$SIGNED_BINARY"
...
test_sbsign_signing() {
    ...
    printf '\x4d\x5a' > "$TEST_BINARY"  # MZ header
    dd if=/dev/zero bs=1024 count=1 >> "$TEST_BINARY" 2>/dev/null
    ...
    output=$(OPENSSL_CONF=/dev/null sbsign \
        --key "$KEY_ID" \
        --cert "$CERT_FILE" \
